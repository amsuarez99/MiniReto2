...
void join(char *message) {
    ...
    while (ocurr != NULL)
    {
        while (pcurr != NULL)
        {
            if (strcmp(ocurr->order->pid, pcurr->product->pid))
            {
...
int main(int argc, char *argv[])
{
    ...
    fileProducts = readProducts();
    fileOrders = readOrders();
        ...
        while (recv(sock, client_message, 1, 0) > -1)
        {
            ...
            case INSERT:
                ...
                if (strcmp(client_message, "orders") == 0)
                {
                    ...
                    fprintf(fileOrders, "%s", client_message);
                    fflush(fileOrders);
                ...
                else if (strcmp(client_message, "products") == 0)
                {
                    ...
                    fprintf(fileProducts, "%s" , client_message);
		     fflush(fileProducts);
